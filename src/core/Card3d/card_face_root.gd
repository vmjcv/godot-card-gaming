...
const _CARD_FACE_SHADER_FILE = CFConst.PATH_CARD_3D + "card_face.shader"
const _CARD_FACE_SHADER = preload(_CARD_FACE_SHADER_FILE)
...
func change_nodes_material(all_nodes,color):
	for n in all_nodes:
		if n is VisualInstance:
			...
			n.material_override.shader = _CARD_FACE_SHADER
