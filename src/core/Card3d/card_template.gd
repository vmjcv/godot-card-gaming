...
func _init_card_3d():
	var _CARD_FRONT_SCENE_FILE = load(CFConst.PATH_CARD_3D_MODEL_FRONT + "%d/scene.glb"%card_front_type)
	var front_scene1 = _CARD_FRONT_SCENE_FILE.instance()
	var front_scene2 = _CARD_FRONT_SCENE_FILE.instance()
	$"卡牌/卡面/卡面模型".add_child(front_scene1)
	$"卡牌/卡面/Viewport/Spatial".add_child(front_scene2)

	
	var _CARD_CENTER_SCENE_FILE = load(CFConst.PATH_CARD_3D_MODEL_CENTER + "%d/scene.glb"%card_center_type)
	var center_scene1 = _CARD_CENTER_SCENE_FILE.instance()
	$"卡牌/卡内/Viewport/Spatial".add_child(center_scene1)
	
	var _CARD_BACK_SCENE_FILE = load(CFConst.PATH_CARD_3D_MODEL_BACK+ "%d/scene.glb"%card_back_type)
	var back_scene1 = _CARD_BACK_SCENE_FILE.instance()
